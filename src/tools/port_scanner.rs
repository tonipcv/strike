...
impl PortScanner {
    ...
    pub fn with_concurrency(timeout_ms: u64, max_concurrent: usize) -> Self {
        Self { 
            timeout_ms,
            max_concurrent,
        }
            ...
            let tasks: Vec<_> = chunk
                ...
                .map(|&port| {
                    ...
                    tokio::spawn(async move {
                        let socket_addr = SocketAddr::new(ip_clone, port);
                        match timeout(
    ...
    pub async fn scan_range(&self, ip: IpAddr, start_port: u16, end_port: u16) -> Result<Vec<u16>> {
        let ports: Vec<u16> = (start_port..=end_port).collect();
        self.scan_ports(ip, ports).await
    }
    
    pub async fn scan_top_1000(&self, ip: IpAddr) -> Result<Vec<u16>> {
        // Top 1000 most common ports (abbreviated for brevity)
        let top_ports = vec![
        ...
        self.scan_ports(ip, top_ports).await
    }
}
